...
func TestCreateReleaseSuccess(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"id":42,"html_url":"https://github.com/owner/repo/releases/tag/v1.2.3"}`))
	}))
	defer server.Close()
	...
	if release.ID != 42 || release.URL != "https://github.com/owner/repo/releases/tag/v1.2.3" {
